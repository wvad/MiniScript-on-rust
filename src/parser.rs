...
fn parse_value_expr(tokens: &mut VecDeque<Token>) -> Result<Expression, String> {
  if let Some(token) = tokens.pop_front() {
    match token.kind {
      ...
      TokenKind::NumLiteral(value) => {
        return Ok(NumberValue(value.value))
      },
      ...
      TokenKind::LeftParen => {
        ...
        if token.kind != TokenKind::RightParen {
          return Err(format!("Expected ')' but found '{:?}'", token.kind));
        }
        expr
      },
      _ => return Err(format!("Expected primary but found '{:?}'", token.kind))
    }
  } else {
        ...
        while let Some(token) = tokens.front() {
          ...
          if let Some(token) = tokens.front() {
            if token.kind == TokenKind::Comma {
              ...
              return Err(format!("Expected ',' or ')' but found '{:?}'", token.kind));
            }
          } else {
            return Err("Unexpected end of input".to_string());
          }
...
fn parse_logical_and(tokens: &mut VecDeque<Token>) -> Result<Expression, String> {
  ...
  while tokens.front().map(|token| {
    token.kind == TokenKind::DoubleAnd
  }).unwrap_or(false) {
    tokens.pop_front();
...
fn parse_logical_or(tokens: &mut VecDeque<Token>) -> Result<Expression, String> {
  ...
  while tokens.front().map(|token| {
    token.kind == TokenKind::DoublePipe
  }).unwrap_or(false) {
    tokens.pop_front();
...
fn parse_assignment(tokens: &mut VecDeque<Token>) -> Result<Expression, String> {
  ...
  if let Some(token) = tokens.front() {
    if token.kind == TokenKind::SingleEqual {
      tokens.pop_front();
      return Ok(Assignment(left.boxing(), parse_assignment(tokens)?.boxing()))
    }
  }
  Ok(left)
}
